...
void GPU::WriteGP1(u32 value)
{
  ...
  switch (command)
  {
    ...
    case static_cast<u8>(GP1Command::SetDisplayStartAddress):
    {
      ...
      {
        ReadVRAM(0, 0, VRAM_WIDTH, VRAM_HEIGHT);
        Screenshot3D::UpdateFromVRAM(g_vram);
      }
...
bool GPU::HandleRenderPolygonCommand()
{
  ...
  if (Screenshot3D::WantsPolygon())
  {
    ...
    for (u32 i = 0; i < (rc.quad_polygon ? 4u : 3u); i++)
    {
      v[i].color = (shaded && i > 0) ? (FifoPeek(k++) & UINT32_C(0x00FFFFFF)) : first_color;
      const u64 maddr_and_pos = s_locals.fifo.Peek(k++);
      ...
      if (use_pgxp)
      {
        use_pgxp = CPU::PGXP::GetPreciseVertexFor3DScreenshot(
          Truncate32(maddr_and_pos >> 32), vp.bits, s_locals.drawing_offset.x + vp.x, s_locals.drawing_offset.y + vp.y,
          s_locals.drawing_offset.x, s_locals.drawing_offset.y, &pgxp_v[i][0], &pgxp_v[i][1], &pgxp_v[i][2]);
      }
    }

    Screenshot3D::DrawPolygon(rc, v, s_locals.draw_mode.mode_reg, s_locals.draw_mode.palette_reg,
                              s_locals.draw_mode.texture_window, use_pgxp ? pgxp_v : nullptr);
  }
...
bool GPU::HandleRenderRectangleCommand()
{
  ...
  if (Screenshot3D::WantsRectangle())
  {
    ...
    const GPUVertexPosition vp{s_locals.fifo.Peek(k++)};
    v.x = TruncateGPUVertexPosition(vp.x);
    v.y = TruncateGPUVertexPosition(vp.y);

    v.texcoord = rc.texture_enable ? Truncate16(s_locals.fifo.Peek(k++)) : 0;
    ...
    switch (rc.rectangle_size)
    {
      ...
      default:
      {
        const u32 width_and_height = s_locals.fifo.Peek(k++);
    ...
    {
      Screenshot3D::DrawRectangle(rc, v, width, height, s_locals.draw_mode.mode_reg, s_locals.draw_mode.palette_reg,
                                  s_locals.draw_mode.texture_window);
    }
  }
